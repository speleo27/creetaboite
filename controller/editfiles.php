...
case 3:
    ...
    //die();
    //appel du document 
    $file=file_get_contents("../pdf/matrice/avant_projet.html");

    //modif du document pour le remplir
    $file=str_replace("{{{REF_PROSPECT}}}",strtoupper($loc['societe_ref_prosp']),$file);
    ...
    $file=str_replace("{{{Code_SS}}}",strtoupper($loc['rcs_city']),$file);
    ...
    $file=str_replace("{{{Code_SS2}}}",$loc['customer_zip_code'],$file);
    $file=str_replace("{{{Date_de_signature}}}",DateFormat($loc['date_entrance']),$file);
    $file=str_replace("{{{Date_de_fin}}}",DateFormat($loc['date_end']),$file);
    $file=str_replace("{{{BAIL}}}",DateFormat($loc['date_end']),$file);
    $file=str_replace("{{{CONVENTION}}}",DateFormat($loc['date_end']),$file);
    ...
    $file=str_replace("{{{Loyer__m2}}}",$loc['rent_surface_price_m'],$file);
    ...
    $mpdf->WriteHTML($file);
    $mpdf->Output("../upload/".$_POST['societe_ref_prosp']."/avant-projet".$_POST['societe_ref_prosp']."_".$time.".pdf",\Mpdf\Output\Destination::FILE);
    $mpdf->Output("../upload/".$_POST['societe_ref_prosp']."/avant-projet".$_POST['societe_ref_prosp']."_".$time.".pdf",\Mpdf\Output\Destination::DOWNLOAD);
    $name="avant-projet".$_POST['societe_ref_prosp']."_".$time.".pdf";
    $req=$bdd->prepare("INSERT INTO upload(societe_ref_prosp,upload_doc_name,upload_doctype_id) VALUES (?,?,?)");
    $req->execute(array($_POST['societe_ref_prosp'],$name,17));
break;

case 4:
        // Bail dérogatoire 12 mois
    // la requete necessaire pour le document
    $reqloc=$bdd->prepare("SELECT * FROM societe INNER JOIN customer ON(customer.societe_ref_prosp = societe.societe_ref_prosp) LEFT JOIN local on (local.ref_int_local = societe.ref_int_local) LEFT JOIN lease_term on(lease_term.lease_term_id = societe.lease_term_id)  WHERE societe.societe_ref_prosp=?");
    $reqloc->execute(array($_POST['societe_ref_prosp']));
    $loc=$reqloc->fetch();
    //appel du document 
    $file=file_get_contents("../pdf/matrice/bail_derog_12mois.html");
    ...
    $mpdf->Output("../upload/".$_POST['societe_ref_prosp']."/bail-derogatoire-12mois".$_POST['societe_ref_prosp']."_".$time.".pdf",\Mpdf\Output\Destination::FILE);
    $mpdf->Output("../upload/".$_POST['societe_ref_prosp']."/bail-derogatoire-12mois".$_POST['societe_ref_prosp']."_".$time.".pdf",\Mpdf\Output\Destination::DOWNLOAD);
    $name="bail-derogatoire-12mois".$_POST['societe_ref_prosp']."_".$time.".pdf";
    $req=$bdd->prepare("INSERT INTO upload(societe_ref_prosp,upload_doc_name,upload_doctype_id) VALUES (?,?,?)");
...
case 5:
    ...
    $loc=$reqloc->fetch();
    //appel du document 
    $file=file_get_contents("../pdf/matrice/bail_derog_24mois.html");
...
case 6:
         ...
         $loc=$reqloc->fetch();
         //appel du document 
         $file=file_get_contents("../pdf/matrice/bail.html");
